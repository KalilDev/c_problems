...
int main() {
  while (true) {
    ...
    printf("Digite o campo minado, com %c sendo espaços vazios e %c sendo "
           "bombas\n",
           xis, bomba);
    ...
    for (int jogada = 0; jogada < n_jogadas; jogada++) {
      ...
      for (int p = -1; p <= 1; p++) {
        ...
        for (int q = -1; q <= 1; q++) {
          ...
          tem_bomba_vizinha =
              tem_bomba_vizinha || (tabuleiro[i + p][j + q] == bomba);
        }
      }
  ...
  prompt_retry:
    ...
    for (char should_retry = getchar(); should_retry != EOF;
         should_retry = getchar()) {
